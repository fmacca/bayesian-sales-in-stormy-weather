...
data = list(y = ar7data[,c(2,3,4,5,6)],
            ...
            z2 = ar7data[,c(45,46,47,48,49)],
            z3 = ar7data[,c(50,51,52,53,54)],
            z4 = ar7data[,c(55,56,57,58,59)],
            z5 = ar7data[,c(60,61,62,63,64)],
            z6 = ar7data[,c(65,66,67,68,69)],
            ...
            zstar2 = ar7data.test[,c(45,46,47,48,49)],
            zstar3 = ar7data.test[,c(50,51,52,53,54)],
            zstar4 = ar7data.test[,c(55,56,57,58,59)],
            zstar5 = ar7data.test[,c(60,61,62,63,64)],
            zstar6 = ar7data.test[,c(65,66,67,68,69)],
            zstar7 = ar7data.test[,c(70,71,72,73,74)],
            nstar = dim(ar7data.test)[1])

modelRegress=jags.model("multivariate_wishart_7.bug",data=data,n.adapt=1000,n.chains=1)
update(modelRegress,n.iter=10000)

variable.names=c("a1", "a2", "a3", "a4", "a5", "a6", "a7", "b1","b2","b3","b4","b5", "tau","ystar")
n.iter=50000 
thin=10
...
head(data.out_wish)
save(data.out_wish,list="data.out_wish",file="output/data.out_wish_7.RData")
...
alpha7.post <- data.out_wish[,151:175]
beta.post <- data.out_wish[,176:200]
sigma.post <- data.out_wish[,201:225]
ystar.post <- data.out_wish[,226:275]
