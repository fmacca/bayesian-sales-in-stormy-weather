...
PlotBstsForecastDistribution(fit, burn = burnin,show.actuals = FALSE)
lines(working_dataset$date,working_dataset$units,col = 'green4')

PlotBstsPredictionErrors(fit,burn=burnin)
...
PlotBstsForecastDistribution(fit3,col.actuals = 'green4')
PlotBstsPredictionErrors(fit3,burn=burnin)
plot(-residuals(fit3,mean.only = TRUE))

pred=predict(fit3,horizon = 30)
plot(pred)
...
par(mfrow=c(1,1))
plot(fit4,burn=burnin)#,show.actuals=FALSE)
lines(working_dataset$date,working_dataset$units,col = 'green4')
lines(working_dataset$date,rep(0,length(working_dataset$date)),col='red')

PlotBstsForecastDistribution(fit4,col.actuals = 'green4')
pred=predict(fit4,horizon = 30)
plot(pred)
...
par(mfrow=c(1,1))
err5=colMeans(bsts.prediction.errors(fit5,burn = burnin)$in.sample)
...
summary(model_components <- AddLocalLinearTrend(model_components, 
                                                y = y))
summary(model_components <- AddAr(model_components, 
                                  y = y,lags = 6,sigma.prior = SdPrior(sigma.guess=3*sdy,
                                                                       upper.limit=sdy,
                                                                       sample.size=length(y))))
# summary(model_components <- AddRandomWalkHoliday(model_components, 
#                                                  y = y,holiday=july4))
# summary(model_components <- AddRandomWalkHoliday(model_components, 
#                                                  y = y,holiday=christmas))
fit6 <- bsts(units ~ .,data=data.zoo,state.specification=model_components, niter = niter,model.options = options)
...
PlotBstsForecastDistribution(fit6, burn = burnin,show.actuals = FALSE)
lines(working_dataset$date,working_dataset$units,col = 'green4')

PlotBstsPredictionErrors(fit6,burn=burnin)

PlotBstsPredictors(fit6,burn=burnin)
PlotBstsCoefficients(fit6,burn=burnin)

PlotBstsComponents(fit6,burn=burnin)

#Model 7: with auto AR
lags=7
(model_components <- list())
summary(model_components <- AddAutoAr(model_components, 
                                  y = y,lags = lags,prior = SpikeSlabArPrior(
                                    lags,
...
colnames(fit7$state.contributions)
par(mfrow=c(1,1))
plot(working_dataset$date,working_dataset$units,col = 'green4', type='l')
# lines(working_dataset$date,colMeans(fit$state.contributions[-(1:burnin),"trend",]),col='red')
# lines(working_dataset$date,colMeans(fit$state.contributions[-(1:burnin),"seasonal.7.1",]),col='blue')
# lines(working_dataset$date,colMeans(fit$state.contributions[-(1:burnin),"seasonal.30.1",]),col='orange')
# lines(working_dataset$date,colMeans(fit$state.contributions[-(1:burnin),"Monthly",]),col='black')
lines(working_dataset$date,colMeans(fit7$state.contributions[-(1:burnin),"Ar7",]),col='black')
# lines(working_dataset$date,colMeans(fit$state.contributions[-(1:burnin),"July4",]),col='red')
# lines(working_dataset$date,colMeans(fit$state.contributions[-(1:burnin),"Christmas",]),col='red')

PlotBstsForecastDistribution(fit7, burn = burnin,show.actuals = FALSE)
lines(working_dataset$date,working_dataset$units,col = 'green4')

PlotBstsPredictionErrors(fit7,burn=burnin)

par(mfrow=c(1,1))
plot(fit7,burn=burnin)
lines(working_dataset$date,working_dataset$units,col = 'green4')
lines(working_dataset$date,rep(0,length(working_dataset$date)),col='red')
...
for(i in 1:lags)
{
  chain <- fit7$AR7.coefficients[-(1:burnin),i]
  layout(matrix(c(1,2,3,3),2,2,byrow=T))
  plot(chain,type="l",main=paste("Trace plot of a",i))
  acf(chain,lwd=3,col="red3",main=paste("autocorrelation of a",i))
  hist(chain,nclass="fd",freq=F,main=paste("Posterior of a",i),col="gray") 
...
CompareBstsModels(model.list=list(fit,fit2,fit3,fit4,fit5,fit6,fit7))
