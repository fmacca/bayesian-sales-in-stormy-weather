...
train <- train %>% filter(train$store_nbr == 32, train$item_nbr == 19) # item 19, store 32
key <- read.table('../../Data/key.csv', header = TRUE, sep = ',') # store 32 --> station 13
load('../../Dataset_pronti/weather_temp.RData')
meteo <- weather %>% filter(weather$station_nbr == 13)
weather_temp <- read.table('../../Data/weather_temp.csv', header = TRUE)
meteo_temp <- weather_temp %>% filter(weather_temp$station_nbr == 13)

# integrate train dataset (units sold of product 19 in store 32) with meteo 
index <- rep(0,24)
j = 1
k = 1
...
# substitute NA occurrences in tavg with the mean value of tmax and tmin
na_ind <- which(is.na(mydata$tavg)==TRUE)
mydata$tavg[na_ind] = (mydata$tmax[na_ind] + mydata$tmin[na_ind]) / 2

# change variable snow with dayBeforeSnow (more meaningful)
aux <- c(mydata$snow[2:dim(mydata)[1]],0)
mydata$snow <- aux
...
# mydata$wetbulb[365] is NA 
# do we use this variable in our model?
# we keep it out in this first analisys

acf(mydata$units)
pacf(mydata$units)

### Begin with a ZIP model
...
par(mfrow=c(1,3))
acf(data.out[,'a.1.'],lwd=3,col="red3",main="autocorrelation of a0")
acf(data.out[,'a.2.'],lwd=3,col="red3",main="autocorrelation of a1")
acf(data.out[,'a.3.'],lwd=3,col="red3",main="autocorrelation of a2")

par(mfrow=c(2,4))
acf(data.out[,'b.1.'],lwd=3,col="red3",main="autocorrelation of b0")
...
acf(data.out[,'b.5.'],lwd=3,col="red3",main="autocorrelation of b4")
acf(data.out[,'b.6.'],lwd=3,col="red3",main="autocorrelation of b5")
acf(data.out[,'b.7.'],lwd=3,col="red3",main="autocorrelation of b6")
acf(data.out[,'b.8.'],lwd=3,col="red3",main="autocorrelation of b7")
...
plot(perc0, type = "b", col = "orange1", lwd = 6, 
     main = "Sample distribution vs Predictive distribution", 
     sub = "where units differ from 0", ylab = "%", xlab = "units", 
...
par(mfrow=c(2,4))
acf(data.outP[,'b.1.'],lwd=3,col="red3",main="beta_0")
acf(data.outP[,'b.2.'],lwd=3,col="red3",main="beta_1")
acf(data.outP[,'b.3.'],lwd=3,col="red3",main="beta_2")
acf(data.outP[,'b.4.'],lwd=3,col="red3",main="beta_3")
acf(data.outP[,'b.5.'],lwd=3,col="red3",main="beta_4")
acf(data.outP[,'b.6.'],lwd=3,col="red3",main="beta_5")
acf(data.outP[,'b.7.'],lwd=3,col="red3",main="beta_6")
acf(data.outP[,'b.8.'],lwd=3,col="red3",main="beta_7")
...
layout(matrix(c(1,1,3,4,2,2),2,3,byrow=T))
...
lines(density(ysdP),col="hotpink4",lwd=2)
abline(v=quantile(ysdP,prob=c(0.025)),col="tan1",lty=2,lwd=2)
abline(v=quantile(ysdP,prob=c(0.5)),col="tan1",lty=1,lwd=2)
abline(v=quantile(ysdP,prob=c(0.975)),col="tan1",lty=2,lwd=2)
abline(v=sd(mydata$units), col = "cyan", lty = 1, lwd = 5)
#legend("topright",legend=c("posterior median", "95% Credible bounds","sample std deviation"),
#       lwd=c(2,2,5), col=c("red","red","limegreen"),lty=c(1,2,1))

plot(0,0, axes = F, xlab = "", ylab = "")
legend("center",legend=c("posterior median", "95% Credible bounds","sample mean"),
       lwd=c(2,2,5), col=c("red","red","limegreen"),lty=c(1,2,1))

plot(0,0, axes = F, xlab = "", ylab = "")
legend("center",legend=c("posterior median", "95% Credible bounds","sample std deviation"),
       lwd=c(2,2,5), col=c("tan1","tan1","cyan"),lty=c(1,2,1))


plot(1:365, lambdamodP, type = 'l', lty = 1, lwd = 1.5, col = "red3", xlab = "days",
...
# 
# test1932 <- read.table("test_1932.txt", header=TRUE)
# 
# ### Our Data vs Predictive Distribution
# n.chain <- dim(data.outP)[1]
# PredP_test <- matrix(ncol=dim(test1932)[1],nrow=n.chain)
# for(i in 1:n.chain){
#   PredP_test[i,]= beta.postP[i,1] + beta.postP[i,2] * test1932$tavg + 
#     beta.postP[i,3] * test1932$foschia + beta.postP[i,4] * test1932$snow + 
#     beta.postP[i,5] * test1932$weekend + beta.postP[i,6] * test1932$avgspeed +
#     beta.postP[i,7] * test1932$holiday + beta.postP[i,8] * test1932$thunder
# }
# PredP_test = exp(PredP_test)
# ypredP_test = rep(0,n.chain * dim(test1932)[1])
# k = 1
# for(i in 1:n.chain){
#   for(j in 1:dim(test1932)[1]){
#     ypredP_test[k] = rpois(1,PredP_test[i,j])
#     k = k + 1
#   }
# }
# perc0_test <- table(test1932$units)/length(test1932$units)
# perc0predP_test <- table(ypredP_test)/length(ypredP_test)
# 
# par(mfrow=c(1,1))
# plot(perc0_test, type = "b", col = "indianred1", lwd = 6, 
#      main = "Prediction on 2013", ylab = "%", 
#      xlab = "units", xlim = c(0,5), ylim = c(0,0.65))
# lines(perc0predP_test, type = "b", col = "mediumpurple4", lwd = 3, lty = 2)
# legend("topright",legend=c("sample","predictive"),
#        lwd=c(3,3), col=c("indianred1","mediumpurple4"),lty=c(1,2))
# 
# 
# ############################################################
